...
DB_USER = "CONNQ_DEV_DB"
DB_PWD = "Connqtion$#2024"
WALLET_DIR = r"C:\Users\AdityaIyer\wallet"
WALLET_PWD = "Connqtion$#2024"
DSN = (
    "(description= (retry_count=20)(retry_delay=3)"
    "(address=(protocol=tcps)(port=1522)(host=adb.ap-mumbai-1.oraclecloud.com))"
    "(connect_data=(service_name=gb679cfde2e0a96_d2w1n9hojweqjdr2_high.adb.oraclecloud.com))"
...
Note: Use the column names exactly as they exist in the tables.
...
Additionally, if the user query contains a year or date reference (e.g., "2025" or "this year"), ensure that filtering is applied on the appropriate date column as determined from the column descriptions.
*Important*: Do not use unnecessary joins or UNION operations; use only the tables and columns needed to answer the query.
Return only the complete SQL query (which must start with SELECT) as plain text.
Do not include any extra commentary.
Do not add ';' at the end of the SQL query generated.
"""
...
Now verify that:
1. Every column used exists in the provided schema.
2. Only the following tables are referenced: {', '.join(TABLES)}.
3. When filtering by a date or year, the SQL uses the appropriate date column as indicated in the column descriptions.
4. Literal values are formatted correctly based on their data types. (For example, if a column is of a string type, the literal must be enclosed in single quotes.)
5. Only the necessary tables and columns are used to answer the query (avoid unnecessary joins or unions).
If there is any mismatch, revise the SQL accordingly. Otherwise, confirm it is correct.
